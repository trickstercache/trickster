...
func TestPrometheus(t *testing.T) {
	...
	t.Run("range query partial hit", func(t *testing.T) {
		...
		t.Logf("wide request: %s", hdr.Get(headers.NameTricksterResult))
		require.Contains(t, []string{status.StatusPartialHit, status.StatusHit}, result[keys.Status],
			"expected partial hit or hit, got %s", result[keys.Status])
	})
	...
	t.Run("instant query", func(t *testing.T) {
		...
		result := parseTricksterResult(hdr.Get(headers.NameTricksterResult))
		t.Logf("instant query: %s", hdr.Get(headers.NameTricksterResult))
	...
	t.Run("labels", func(t *testing.T) {
		...
		require.Equal(t, "success", pr.Status)
		result := parseTricksterResult(hdr.Get(headers.NameTricksterResult))
		t.Logf("labels: %s", hdr.Get(headers.NameTricksterResult))
		require.Equal(t, "ObjectProxyCache", result["engine"])
		var labels []string
	...
	t.Run("series", func(t *testing.T) {
		...
		require.Equal(t, "success", pr.Status)
		result := parseTricksterResult(hdr.Get(headers.NameTricksterResult))
		t.Logf("series: %s", hdr.Get(headers.NameTricksterResult))
		require.Equal(t, "ObjectProxyCache", result["engine"])
	...
	t.Run("label values", func(t *testing.T) {
		...
		require.Equal(t, "success", pr.Status)
		result := parseTricksterResult(hdr.Get(headers.NameTricksterResult))
		t.Logf("label values: %s", hdr.Get(headers.NameTricksterResult))
		require.Equal(t, "ObjectProxyCache", result["engine"])
		var values []string
	...
	t.Run("negative cache", func(t *testing.T) {
		...
		t.Logf("negative cache first: status=%d, X-Trickster-Result=%s",
			resp.StatusCode, resp.Header.Get(headers.NameTricksterResult))
		require.Equal(t, http.StatusBadRequest, resp.StatusCode)

		resp2, err := http.Get(u)
		require.NoError(t, err)
		resp2.Body.Close()
		result := parseTricksterResult(resp2.Header.Get(headers.NameTricksterResult))
		t.Logf("negative cache second: status=%d, result=%v", resp2.StatusCode, result)
		if result[keys.Status] == status.StatusNegativeCacheHit {
	...
	t.Run("POST range query", func(t *testing.T) {
		...
		t.Logf("POST range query: %s", resp.Header.Get(headers.NameTricksterResult))
	...
	t.Run("POST instant queries with different query params return different results", func(t *testing.T) {
		...
		t.Logf("POST query=up: %s", resp1.Header.Get(headers.NameTricksterResult))
		...
		t.Logf("POST query=process_cpu_seconds_total: %s", resp2.Header.Get(headers.NameTricksterResult))
	...
	t.Run("metadata endpoint cached", func(t *testing.T) {
		...
		require.Equal(t, "success", pr.Status)
		result := parseTricksterResult(hdr.Get(headers.NameTricksterResult))
		t.Logf("metadata: %s", hdr.Get(headers.NameTricksterResult))
		require.Equal(t, "ObjectProxyCache", result["engine"])
	...
	t.Run("format_query endpoint cached", func(t *testing.T) {
		...
		require.Equal(t, "success", pr.Status)
		result := parseTricksterResult(hdr.Get(headers.NameTricksterResult))
		t.Logf("format_query: %s", hdr.Get(headers.NameTricksterResult))
		require.Equal(t, "ObjectProxyCache", result["engine"])
	...
	t.Run("parse_query endpoint cached", func(t *testing.T) {
		...
		require.Equal(t, "success", pr.Status)
		result := parseTricksterResult(hdr.Get(headers.NameTricksterResult))
		t.Logf("parse_query: %s", hdr.Get(headers.NameTricksterResult))
		require.Equal(t, "ObjectProxyCache", result["engine"])
	})
	...
	t.Run("scrape_pools endpoint cached", func(t *testing.T) {
		...
		require.Equal(t, "success", pr.Status)
		result := parseTricksterResult(hdr.Get(headers.NameTricksterResult))
		t.Logf("scrape_pools: %s", hdr.Get(headers.NameTricksterResult))
		require.Equal(t, "ObjectProxyCache", result["engine"])
	...
	t.Run("stats=all uses separate cache key", func(t *testing.T) {
		...
		result1 := parseTricksterResult(hdr1.Get(headers.NameTricksterResult))
		t.Logf("query without stats: %s", hdr1.Get(headers.NameTricksterResult))

		paramsStats := url.Values{"query": {"up"}, "stats": {"all"}}
		_, hdr2 := queryTricksterProm(t, tricksterAddr, "prom1", "/api/v1/query", paramsStats)
		result2 := parseTricksterResult(hdr2.Get(headers.NameTricksterResult))
		t.Logf("query with stats=all: %s", hdr2.Get(headers.NameTricksterResult))
		...
		if result1[keys.Status] == status.StatusHit || result1[keys.Status] == status.StatusKeyMiss {
			require.NotEqual(t, status.StatusHit, result2[keys.Status],
	...
	t.Run("native histogram instant query", func(t *testing.T) {
		...
		require.Equal(t, "success", pr.Status)
		result := parseTricksterResult(hdr.Get(headers.NameTricksterResult))
		t.Logf("histogram instant: %s", hdr.Get(headers.NameTricksterResult))
		require.Equal(t, "ObjectProxyCache", result["engine"])
	...
	t.Run("native histogram range query", func(t *testing.T) {
		...
		require.Equal(t, "success", pr.Status)
		result := parseTricksterResult(hdr.Get(headers.NameTricksterResult))
		t.Logf("histogram range: %s", hdr.Get(headers.NameTricksterResult))
		require.Equal(t, "DeltaProxyCache", result["engine"])
	...
	t.Run("native histogram range query cache hit", func(t *testing.T) {
		...
		require.Equal(t, "success", pr.Status)
		result := parseTricksterResult(hdr.Get(headers.NameTricksterResult))
		t.Logf("histogram cache miss: %s", hdr.Get(headers.NameTricksterResult))
