...
In a `rule` config, provide `ingress_req_rewriter_name` and/or `egress_req_rewriter_name` configurations to rewrite the Request using the named Request Rewriter. The rewriter is executed before or after, respectively, it is handled by the Rule (including any modifications made by a matching rule case). The meaning of Ingress and Egress, in this case, are scoped to a Request's traversal through the Rule in which these configuration values exist.
