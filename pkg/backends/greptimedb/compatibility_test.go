...
func corpusAnalyze(zone, sql string) sqlanalyzer.Analysis {
	return analyzer.ForSession(pgwire.SessionView{UTC: zone == "UTC"}).Analyze(sql, time.Time{})
}
...
func TestCompatibilityCorpusCoversGrafanaMacros(t *testing.T) {
	sqlcompat.CheckGrafanaMacros(t, corpusPath)
}
