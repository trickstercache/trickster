...
func (o *scalarBinaryHistogramOperations) ApplyScalarBinary(value any,
	operator string, _ float64, _ bool,
) (any, bool) {
...
func TestScalarBinaryWrapperApplyFloat(t *testing.T) {
	tests := []struct {
		...
		{"sum(up) * 100 + 2", 3, 302, true},
		{"100 / sum(up)", 4, 25, true},
		{"10 - sum(up)", 3, 7, true},
		{"sum(up) ^ 2", 3, 9, true},
		{"sum(up) % 2", 3, 1, true},
		{"sum(up) atan2 2", 3, math.Atan2(3, 2), true},
		{"sum(up) == 3", 3, 3, true},
		{"sum(up) != 3", 2, 2, true},
		{"sum(up) < 3", 2, 2, true},
		{"sum(up) >= 3", 3, 3, true},
		...
		{"sum(up) > 0", 2, 2, true},
		{"sum(up) > 0", -2, 0, false},
		{"sum(up) > bool 0", -2, 0, true},
		{"0 < bool sum(up)", 2, 1, true},
	}
	for _, test := range tests {
		...
		got, keep := wrapper.ApplyFloat(test.value)
...
func TestScalarBinaryWrapperApplyHistogram(t *testing.T) {
	...
	if _, keep := wrapper.ApplyHistogram("histogram", operations); keep {
