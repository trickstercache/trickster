...
func TestParseLimitKAggregation(t *testing.T) {
	tests := []struct {
		...
		{name: "quoted grouping labels",
			query:     "limitk by (\"zone.name\", 'rack-name', `cluster name`) (2, up)",
			wantFound: true, wantK: 2, wantInner: "up",
			wantQuery: "limitk by (\"zone.name\", 'rack-name', `cluster name`) (2, up)",
			wantGroup: AggregationGrouping{Labels: []string{"cluster name", "rack-name", "zone.name"}}},
		{name: "quoted comma", query: "limitk by (\"zone,name\") (2, up)",
			wantFound: true, wantK: 2, wantInner: "up",
			wantQuery: "limitk by (\"zone,name\") (2, up)",
			wantGroup: AggregationGrouping{Labels: []string{"zone,name"}}},
		{name: "quoted grouping escapes",
			query:     "limitk by (\"zone\\x2ename\", \"\\u00e9\") (2, up)",
			wantFound: true, wantK: 2, wantInner: "up",
			wantQuery: "limitk by (\"zone\\x2ename\", \"\\u00e9\") (2, up)",
			wantGroup: AggregationGrouping{Labels: []string{"zone.name", "\u00e9"}}},
		{name: "grouping trailing comma", query: "limitk by (job,) (2, up)",
			wantFound: true, wantK: 2, wantInner: "up",
			wantQuery: "limitk by (job,) (2, up)",
			wantGroup: AggregationGrouping{Labels: []string{"job"}}},
		...
		{name: "grouping leading comma", query: "limitk by (,job) (2, up)"},
		{name: "grouping repeated comma", query: "limitk by (job,,instance) (2, up)"},
		{name: "grouping missing comma", query: "limitk by (job instance) (2, up)"},
		{name: "invalid unquoted grouping label", query: "limitk by (zone.name) (2, up)"},
		{name: "unterminated quoted grouping label", query: "limitk by (\"zone.name) (2, up)"},
		{name: "invalid quoted grouping escape", query: "limitk by (\"zone\\qname\") (2, up)"},
		{name: "empty quoted grouping label", query: "limitk by (\"\") (2, up)"},
	}
