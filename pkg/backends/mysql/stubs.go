...
// but are (currently) unused by the MySQL implementation.

// Series (timeseries.Timeseries Interface) stub funcs

// FastForwardRequest is not used for MySQL and is here to conform to the Proxy Client interface
...
// UnmarshalInstantaneous is not used for MySQL and is here to conform to the Proxy Client interface
func (c *Client) UnmarshalInstantaneous(data []byte) (timeseries.Timeseries, error) {
	return nil, nil
}

// QueryRangeHandler is not used for MySQL and is here to conform to the Proxy Client interface
