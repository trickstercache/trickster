...
[backends]
    ...
    rule_name = ''
        ...
        [backends.test.paths]
            [backends.test.paths.series]
            path = "/series"
            handler = "proxy"
            req_rewriter_name = ''

            [backends.test.paths.label]
            path = "/label"
            ...
            [backends.test.paths.label.response_headers]
        ...
        certificate_authority_paths = [  'file.that.should.not.exist.ever.pem' ]
        client_key_path = 'test_client_key'
        client_cert_path = 'test_client_cert'

`

func fromTestTOML() (*Options, error) {
	return fromTOML(testTOML)
}

func fromTestTOMLWithDefault() (*Options, error) {
	conf := strings.Replace(testTOML, "    rule_name = ''", "    rule_name = ''\n    is_default = false", -1)
	return fromTOML(conf)
}

func fromTestTOMLWithPath() (*Options, error) {
	conf := strings.Replace(testTOML, "    rule_name = ''", "    rule_name = ''\n    is_default = false", -1)
	return fromTOML(conf)
}

func fromTestTOMLWithReqRewriter() (*Options, error) {
	conf := strings.Replace(testTOML, "    rule_name = ''", "    rule_name = ''\n    req_rewriter_name ='test'", -1)
	return fromTOML(conf)
}

func fromTestTOMLWithALB() (*Options, error) {
	conf := strings.Replace(strings.Replace(testTOML, "    rule_name = ''", `
    rule_name = ''
        [backends.test.alb]
