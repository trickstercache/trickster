...
func (c compatibilityCase) analyze(sql string) sqlanalyzer.Analysis {
	return testAnalyze(c.SessionTimeZone == corpusZoneUTC, sql)
}
...
func TestCompatibilityCorpus(t *testing.T) {
	...
	if len(corpus.Omissions) == 0 {
		t.Fatal("the corpus must record what it deliberately leaves out")
	}
	...
	for _, tc := range corpus.Cases {
		t.Run(tc.Name, func(t *testing.T) {
			...
			analysis := tc.analyze(tc.ExpandedSQL)
			...
			assertCompatibilityPlan(t, tc, analysis.Plan)
		})
	}
}

func assertCompatibilityPlan(t *testing.T, tc compatibilityCase, plan *sqlanalyzer.QueryPlan) {
	t.Helper()
	want := tc.Expected
	...
	again := tc.analyze(rendered)
	if again.Mode != sqlanalyzer.CacheModeDelta || again.Plan.CanonicalSQL != plan.CanonicalSQL {
		t.Fatalf("rendering changed the statement:\n%s\n%v / %v", rendered, again.Mode, again.Err)
	}
	if !again.Plan.LowerBound.Value.Equal(lower) || again.Plan.UpperBound == nil ||
		!again.Plan.UpperBound.Value.Equal(end.Add(step)) {
		t.Fatalf("rendered extent reads back as %v..%v, want %v..%v\n%s", again.Plan.LowerBound.Value,
...
func BenchmarkCompatibilityCorpus(b *testing.B) {
	...
	for _, tc := range corpus.Cases {
		b.Run("Analyze/"+tc.Expected.CacheMode+"/"+tc.Name, func(b *testing.B) {
			b.ReportAllocs()
			for b.Loop() {
				_ = tc.analyze(tc.ExpandedSQL)
			}
		})
		plan := tc.analyze(tc.ExpandedSQL).Plan
		if plan == nil {
			continue
		}
