...
func TestRegisterHandlers(t *testing.T) {
	...
	for _, name := range []string{"health", "proxycache", providers.Proxy} {
...
func TestDefaultPathConfigs(t *testing.T) {
	...
	if len(dpc) != 1 {
		t.Fatalf("expected ordered length to be: %d got %d", 1, len(dpc))
	}
	if dpc[0].Path != "/" || dpc[0].HandlerName != providers.Proxy {
		t.Errorf("expected / -> %s, got %s -> %s", providers.Proxy, dpc[0].Path, dpc[0].HandlerName)
	}
}
