...
func TestClassify(t *testing.T) {
	tests := []struct {
		...
		// step-fixing
		{"summarize(a.b, '1h')", StepFixed, time.Hour, 0, true, "", "", 1},
		{"summarize(a.b, 1h, sum)", StepFixed, time.Hour, 0, true, "", "", 1}, // `1h`, `sum` parse as paths but are not leaves
		{"summarize(a.b, '1h', 'sum', false)", StepFixed, time.Hour, 0, true, "", "", 1},
		{"summarize(a.b, '1h', 'sum', alignToFrom=false)", StepFixed, time.Hour, 0, true, "", "", 1},
		...
		{"scale(summarize(a.b, '1h'), 2)", StepFixed, time.Hour, 0, true, "", "", 1},
		{"sumSeries(summarize(a.b, '1h'), summarize(c.d, '1h'))", StepFixed, time.Hour, 0, true, "", "", 2},
		...
		{"sumSeries(summarize(a.b, '1h'), summarize(c.d, '1d'))", StepUnknown, 0, 0, false, ReasonUnknownStep, "summarize", 2},
		{"hitcount(a.b, '1h')", StepUnknown, 0, 0, false, ReasonFunctionNotAllowlisted, "hitcount", 1},
		{"hitcount(a.b, '1h', true)", StepFixed, time.Hour, 0, true, "", "", 1},
		{"hitcount(a.b, '1h', alignToInterval=true)", StepFixed, time.Hour, 0, true, "", "", 1},
		...
		{"summarize(movingAverage(a.b, '5min'), '1h')", StepUnknown, 0, 0, false, ReasonFunctionNotAllowlisted, "movingAverage", 1},
