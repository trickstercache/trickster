...
func (c *Cache) Connect() error {
	c.Logger.Info("badger cache setup", logging.Pairs{"cacheDir": c.Config.Badger.Directory})
...
func (c *Cache) Store(cacheKey string, data []byte, ttl time.Duration) error {
	...
	c.Logger.Debug("badger cache store", logging.Pairs{"key": cacheKey, "ttl": ttl})
...
func (c *Cache) Retrieve(cacheKey string, allowExpired bool) ([]byte, status.LookupStatus, error) {
	...
	if err == nil {
		c.Logger.Debug("badger cache retrieve", logging.Pairs{"key": cacheKey})
	...
	if err == badger.ErrKeyNotFound {
		err = cache.ErrKNF
		c.Logger.Debug("badger cache miss", logging.Pairs{"key": cacheKey})
		metrics.ObserveCacheMiss(cacheKey, c.Name, c.Config.Provider)
		return nil, status.LookupStatusKeyMiss, err
	}

	c.Logger.Debug("badger cache retrieve failed", logging.Pairs{"key": cacheKey, "reason": err.Error()})
	metrics.ObserveCacheMiss(cacheKey, c.Name, c.Config.Provider)
...
func (c *Cache) Remove(cacheKey string) {
	c.Logger.Debug("badger cache remove", logging.Pairs{"key": cacheKey})
...
func (c *Cache) BulkRemove(cacheKeys []string) {
	c.Logger.Debug("badger cache bulk remove", nil)
...
func (c *Cache) SetTTL(cacheKey string, ttl time.Duration) {
	...
	c.Logger.Debug("badger cache update-ttl", logging.Pairs{"key": cacheKey, "ttl": ttl, "success": err == nil})
	if err == nil {
