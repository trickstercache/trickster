...
// Package vitess adapts the Vitess MySQL parser to Trickster's
// dialect-independent sqlanalyzer contract. It provides parsing and analysis
// services for the MySQL wire-protocol backend and any future backend speaking
// a MySQL-compatible dialect; backends consume the resulting
// sqlanalyzer.Analysis and never touch Vitess ASTs themselves.
package vitess
...
// MustNewAnalyzer returns a new Analyzer, panicking if the underlying Vitess
...
// Parser exposes the underlying Vitess parser for protocol-level statement
// handling (splitting, tokenizing, pre-parsing) by consuming backends.
func (a *Analyzer) Parser() *sqlparser.Parser {
	return a.parser
}
...
// AnalyzeParsed analyzes a statement that the caller already ran through this
// analyzer's Parser, avoiding a second parse. The AST is read, never modified.
func (a *Analyzer) AnalyzeParsed(statement string, stmt sqlparser.Statement,
	...
	canonical, extentRenderer, err := buildArtifacts(selectStmt, rng, bucket.step)
	if err != nil {
...
func buildArtifacts(stmt *sqlparser.Select, rng rangeInfo, step time.Duration) (string, *renderer, error) {
	...
	return sqlparser.String(template), &renderer{
		template:   template,
...
func (r *renderer) RenderExtent(extent timeseries.Extent) (string, error) {
	lower := extent.Start
	if !r.lowerInclusive {
...
func (r *renderer) RenderTimeRange(lower, upper time.Time) (string, error) {
